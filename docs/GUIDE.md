...
3. **Concurrent reads, sequential writes.** Source tables are pre-fetched concurrently (up to `--concurrency` at once) into in-memory buffers; the destination is written one table at a time within the transaction.
